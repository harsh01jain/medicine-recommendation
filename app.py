...
MILD_MODEL_PATH = os.getenv("MILD_MODEL_PATH")
MODERATE_MODEL_PATH = os.getenv("MODERATE_MODEL_PATH")
SEVERE_MODEL_PATH = os.getenv("SEVERE_MODEL_PATH")
...
description_data_path = os.getenv("Description")
diet_data_path = os.getenv("Diet")
workout_data_path = os.getenv("Workout")
Medicine_data_path= os.getenv("Medicine")

# Load data from CSV files
description_df = pd.read_csv(description_data_path)
diet_df = pd.read_csv(diet_data_path)
workout_df = pd.read_csv(workout_data_path)
Medicine_df = pd.read_csv(Medicine_data_path,encoding='latin1')
...
def login_required(f):
    ...
    def decorated_function(*args, **kwargs):
        # Check if user is logged in (either normal or Google)
        if "id" not in session and "user" not in session:
...
mild_data_path = os.getenv("mild_data")
moderate_data_path = os.getenv("moderate_data")
severe_data_path = os.getenv("severe_data")

mild_data = pd.read_csv(mild_data_path)
moderate_data = pd.read_csv(moderate_data_path)
severe_data = pd.read_csv(severe_data_path)
...
def index():
    ...
    #  Check session: normal login (id) OR Google login (user)
    if 'id' not in session and 'user' not in session:
        app.logger.info("No user session found. Redirecting to landing page.")
        return redirect(url_for('landing'))

    # Case 1: Google OAuth login
    if 'user' in session:
        user = session['user']
        app.logger.info(f"Google user {user['email']} logged in.")
        flash("Logged in successfully with Google!", "success")
        return render_template('index.html', user_name=user['name'])

    #  Case 2: Normal DB login
    user_id = session['id']
    app.logger.info(f"User session found with id={user_id}")
    ...
    try:
        ...
        if not user:
            app.logger.warning(f"No user found in DB for id={user_id}. Clearing session.")
            session.pop('id', None)
            return redirect(url_for('landing'))

        user_name = user[0]
        app.logger.info(f"User {user_name} found. Rendering dashboard.")
        flash("Logged in successfully!", "success")

        return render_template('index.html', user_name=user_name)

    except Exception as e:
        app.logger.error(f"Database error while fetching user {user_id}: {e}")
        return redirect(url_for('landing'))
...
def test_endpoint():
    return {'success': True, 'message': 'Backend is working'}, 200
...
def test_database():
    try:
        ...
        result = cur.fetchone()
        cur.close()
        return {'success': True, 'message': 'Database connection working', 'result': result[0]}, 200
    except Exception as e:
        app.logger.error(f"Database test failed: {e}")
        return {'success': False, 'message': f'Database test failed: {str(e)}'}, 500
...
def authorize_google():
    ...
    if user:
        user_id = user[0]
        # Optionally update name if changed on Google
        cur.execute("UPDATE users SET name = %s WHERE id = %s", (name, user_id))
    else:
        #  Insert new user (password NULL for Google accounts, auto-verified)
        cur.execute(
            "INSERT INTO users (name, email, password, is_verified, created_at) VALUES (%s, %s, %s, %s, NOW())",
            (name, email, None, 1)
        )
    ...
    #  Store user session (just like manual login)
    session["id"] = user_id
    flash("Logged in successfully!", "success")
...
def dashboard():
    if "user" not in session:
        return redirect(url_for("login"))  # go to normal login page
...
@app.route('/logout')
def logout():
    # Clear both normal login and Google login sessions
    session.pop('id', None)
    session.pop('user', None)
    session.clear()  # optional: clears everything in session
